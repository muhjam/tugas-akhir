...
class SimpleHundredRequestTest {
  constructor() {
  ...
  async runTest() {
    ...
    console.log('🔄 Executing 100 requests...');
    ...
    for (let i = 1; i <= this.totalRequests; i++) {
      ...
      // Progress indicator
      if (i % 10 === 0) {
        const progress = ((i / this.totalRequests) * 100).toFixed(1);
        const successCount = this.results.filter(r => r.success).length;
        console.log(`📊 Progress: ${i}/${this.totalRequests} (${progress}%) - Success: ${successCount}`);
      }
    }
  ...
  generateReport() {
    ...
    // MAIN SUMMARY
    console.log('📈 TEST RESULTS SUMMARY (100 REQUESTS)');
    console.log('=' .repeat(60));
    ...
    console.log('⏱️  RESPONSE TIME STATISTICS (in milliseconds)');
    console.log('=' .repeat(60));
    ...
    console.log('=' .repeat(60));
    const timeRanges = [
      { min: 0, max: 1000, label: '0-1 second' },
      { min: 1000, max: 2000, label: '1-2 seconds' },
      { min: 2000, max: 5000, label: '2-5 seconds' },
      { min: 5000, max: 10000, label: '5-10 seconds' },
      { min: 10000, max: 30000, label: '10-30 seconds' },
      { min: 30000, max: Infinity, label: '30+ seconds' }
    ];
    ...
    console.log('📋 DETAILED RESULTS (First 20 Requests)');
    console.log('=' .repeat(60));
    ...
    if (this.results.length > 20) {
      console.log(`... and ${this.results.length - 20} more requests`);
    }
    console.log('');
    ...
    console.log('=' .repeat(60));
    console.log(`📊 Total 100 requests completed in ${(totalTime / 1000).toFixed(2)} seconds`);
    console.log(`✅ ${successfulRequests.length} requests successful (${successRate.toFixed(1)}%)`);
    console.log(`⚡ Average response time: ${avgResponseTime.toFixed(0)}ms`);
    console.log(`🚀 Throughput: ${requestsPerSecond} requests/second`);
    console.log('');
    console.log('🎉 100 request test completed!');
  }
}
...
async function runTest() {
  ...
  console.log('✅ Server is running! Starting 100 request test...\n');
  
  const test = new SimpleHundredRequestTest();
  await test.runTest();
}
